...
counties_sf_NE <- st_read('../Data/Spatial/cb_2017_us_county_20m.shp') %>% 
  filter(STATEFP == 31)
...
> ANSWER: EPSG Code is 4269. The counties database uses the NAD83, or the North American 1983 Datum and it is a projected coordinate system. 
...
names(NWIS_NE_sf)
class(NWIS_NE_sf$geometry)

```
> QUESTION: What new field(s) appear in the sf dataframe created? What field(s), if any, disappeared?
> ANSWER: There is now a new column called "geometry", which combined the long and lat columns from the dataframe. The sf dataframe now has one less column that the original dataframe because the sf data frame combined two into one.
...
```{r}
ggplot() + 
  geom_sf(data = counties_sf_NE, col="red") + 
...
gagedata <- NWIS_NE_sf %>%
  left_join(y=flowdata, by = c("site_no" = "site_no"))

#Convert back to sf dataframe
st_crs(gagedata) #it looks like it already is, but I'll do it again just in case
gagedata_sf <- st_as_sf(gagedata, coords = 'geometry',crs=4269)
```
...
#Plot the values
ggplot() + 
  geom_sf(data = counties_sf_NE, col="#004529", size=0.5) + 
  geom_sf(data = gagedata_sf, aes(color=gage_ht)) +
  scale_color_gradient("gage_ht", low='#7fcdbb',high='#081d58')
...
counties_sf_NE_utm <- st_transform(counties_sf_NE, c=32614)
gagedata_sf_utm <- st_transform(gagedata_sf, c=32614)

#Plot the data
ggplot() + 
  geom_sf(data = counties_sf_NE_utm, col="#004529", size=0.5) + 
  geom_sf(data = gagedata_sf_utm, aes(color=gage_ht)) +
  scale_color_gradient("gage_ht", low='#7fcdbb',high='#081d58')

```
> QUESTION: The shape of Nebraska should look a bit different than the one created in Step 5? Why?
> ANSWER: The projection is different. It has different bounds compared to the previous one we were using. For example, now it is in WGS84 instead of NAD83
...
```{r}
#Select the county
counties_sf_NE_utm_Lan <- counties_sf_NE_utm %>%
  filter(NAME=='Lancaster')

#Select gages within 
Lancastermask <- st_intersects(gagedata_sf_utm, counties_sf_NE_utm_Lan, sparse=FALSE)
Lancastergages <- gagedata_sf_utm[Lancastermask,]

#Plot
ggplot() +
  geom_sf(data = counties_sf_NE_utm, col="#004529", size=0.5) +
  geom_sf(data = counties_sf_NE_utm_Lan, col="red") +
  geom_sf(data = Lancastergages, col="#081d58", size = 0.5)
...
#leaflet only does WGS84
counties_sf_NE_wgs <- st_transform(counties_sf_NE, c=4326)
gagedata_sf_wgs <- st_transform(gagedata_sf, c=4326)

#Join
countygages <-  
  st_join(gagedata_sf_wgs, counties_sf_NE_wgs, join = st_intersects)


#Show column names
names(countygages$NAME)
countygages$NAME <- as.character(countygages$NAME) #change to character
counties_sf_NE_wgs$NAME <- as.character(counties_sf_NE_wgs$NAME)
#Plot

leaflet() %>%
  addTiles() %>%
  addPolygons(data=counties_sf_NE_wgs, color="#004529", weight = 1, smoothFactor = 0.5, opacity = 1.0) %>%
  addCircleMarkers(data=countygages,
                   radius=(~gage_ht), 
                   stroke = FALSE, 
                   fillOpacity = 0.3,
                   popup = ~as.character(`NAME`))
#cannot figure out how to color my counties, so the below colors by counties

ggplot(legend=FALSE) +
  geom_sf(data = counties_sf_NE, aes(color = "#004529")) +
  geom_sf(data = countygages, aes(color = NAME, size = gage_ht))
...
```{r}
#Group and summarize
countygages_bycounty <- countygages %>%
  group_by(NAME) %>%
  filter(!is.na(gage_ht)) %>%
  summarise(mean_gage_ht=mean(gage_ht))
View(countygages_bycounty)


#Convert result to a simple dataframe
countygages_bycounty <- st_drop_geometry(countygages_bycounty)

#Join summary to County fc
countygages_join <- counties_sf_NE_wgs %>%
  left_join(countygages_bycounty, by = c("NAME" = "NAME"))

#Plot
ggplot() +
  geom_sf(data = countygages_join, aes(fill= mean_gage_ht)) +
  scale_fill_gradient(low ="#7fcdbb", high="#081d58")
