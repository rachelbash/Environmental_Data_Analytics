...
> It is unique to this computer. So sharing files won't work well with this.
...
#not efficient. Will put data directly into console. Dumb.
...
> ANSWER: Shows you where you retrieved the raw data and how you got the exact csv file. Helps you figure out what each of the columns in the dataset means.
...
str(USGS.flow.data) #structure, gives you class of each column.
dim(USGS.flow.data) #dimensions, number of rows and columns
length(USGS.flow.data) #length, number of columns

head(USGS.flow.data)
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #indexes for rows 30000-30005 and columns 3, 8, and 14

class(USGS.flow.data$datetime)
class(USGS.flow.data$discharge.mean) #numeric
class(USGS.flow.data$discharge.mean.approval) #factor
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data)
summary(USGS.flow.data$discharge.mean) #summary based on numeric elements
summary(USGS.flow.data$discharge.mean.approval) #summary based on factored elements
summary(USGS.flow.data$gage.height.mean)

```

What happened to blank cells in the spreadsheet when they were imported into R?
> ANSWER: NA's were placed in there instead
