...
format(today, format = c("%Y","%B","%d"))
format(today, format = c("%m","%d"))
format(today, format = c("%A","%B","%d","%Y"))

#tells R that the format below is what it was read in as. Then it will rewrite the date into year month day.
...
```{r}
# deletes all the dashes in the dates(and 2018->18), so the first date is now 180101
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# function that adds 19 to date if date is greater than 12/31/18. Paste0 amends something to beginning of string. Take two things and put them together.
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
#applying the function to the datetime column, which adds 19 to 180101
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)
class(USGS.flow.data$datetime) #shows that it is a character

#changes format back into date. Adds dashes back in so it is easier to read as a Date with 1918 as first year
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y%m%d") 
class(USGS.flow.data$datetime) #will show "Date"
```
...
> Answer: Removing NAs would be helpful if . Keeping NA is good because the NA might actually provide valuable data. Be careful that there could be true NAs and black spaces which could also be considered NAs. If there is a threshold to detect and it falls below threshold, NAs would be inputed. 

```{r}
#create new df that has NAs omitted
USGS.flow.data.complete <- na.omit(USGS.flow.data)
dim(USGS.flow.data)
dim(USGS.flow.data.complete)
#complete.cases tells you which cases are complete
complete.cases(USGS.flow.data)

#summary statistics. Only includes values that have a number in them
...
#aes is giving some aesthetics, then tell it to give you a bar plot
ggplot(USGS.flow.data.complete, aes(x = discharge.mean.approval)) +
  geom_bar()
#A=approved, P=provisional data subject to revision
#geom bar works great for categoical variables 
```
...
```{r, fig.height = 3, fig.width = 4}
#tell which plot to plot and how. Discharge mean is continuous quantitative variable
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean))

# pick better binwidth, zooms in so that it goes by every 10 units
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#bins is just the number of bins total in the whole plot
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#add command to create continuous x axis scale that only goes from 0-500. Zooms in on distribution in first 500 rows
ggplot(USGS.flow.data.complete, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
#looking at gage height now
ggplot(USGS.flow.data.complete) +
...
```{r, fig.height = 3, fig.width = 4}
#can create a lines for different variables
ggplot(USGS.flow.data.complete) +
...
#more minimum counts at the lower numbers, then drops and you have a fewer count of minimum values as you go up.

# graphing gage height, and then separating by whether or not they were approved (separate by another variable). One quantitative measurement divided by categorical variables
ggplot(USGS.flow.data.complete) +
  geom_freqpoly(aes(x = gage.height.mean, color = gage.height.mean.approval), 
  bins = 50) +
  scale_x_continuous(limits = c(0, 10)) +
...
```{r, fig.height = 3, fig.width = 4}
#boxplot plots gage height with two categorical variables
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#to divide up two continuous quantitative variables with box plot. Useful if you have non-linear patterns, you can see it better with box plot
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# Violin, the greater the amount of samples, the wider the plot. Lines are the quantiles
ggplot(USGS.flow.data.complete) +
...
#shows that there is a positive correlation between discharge and gage height. Can predict discharge from gage height based on this relationship (and model)
```
...
> ANSWER: Gage height and discharge are positively related
