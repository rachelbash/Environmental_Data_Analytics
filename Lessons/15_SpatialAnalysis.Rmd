...
*geometry: where they are on the map
*attribute: properties of that feature
*coordinate reference sys: 
...
ggplot(EPAair_PM_avg, aes(x=SITE_LONGITUDE, y=SITE_LATITUDE)) + 
  geom_point(aes(color=meanPM), size=4) + coord_equal() #coor-equal scales the map better so you can kind of see the map of NC
```
...
myMap  <- leaflet(data=EPAair_PM_avg) %>% 
  addTiles() %>%  #tiles are the background maps (default is open street map)
  addCircleMarkers(~SITE_LONGITUDE,~SITE_LATITUDE,
                   radius=(~meanPM*2),
                   stroke = FALSE, 
                   fillOpacity = 0.3,
                   popup = ~as.character(`Site.Name`)) #can specify what pops up when you click on point
#Show the map
myMap

#you can either publish (far right above plot) or you can create a markdown in an html and you can have an interactive map
```
...
Durham_buffer <- EPAair_PM_avg_sf %>% 
  ...
  st_buffer(0.1) #in degrees

#View the result
mapView(Durham_buffer)
```

In running the above, you get a warning and the shape looks elliptical, not circular. What's up? Well, it's time to chat about coordinate reference systems...
...
st_crs(EPAair_PM_avg_sf) #4 letter code is the code that says what the crs is, also specifies the datum (model of the shape of the earth) 
```
...
DA_UTM_buffer <- EPAair_PM_avg_sf_UTM %>%     #popular for non universal scales
...
#looks a lot better (more circular, smaller circle) because we chose a better projection
```
...
plot(huc8_sf)
plot(huc8_sf['SUB'], #only does SUB column
  graticule = st_crs(4326), #gives its a ref system so it looks like its on a sphere
  axes=TRUE)
...
mapview(state_sf2)
#this is a list, just note the difference between the two functions
```
...
#leaflets don't let you click on anything
#leaflets only do WGS84 (which is a geographic coordinate system)
#
#if you want to do overlays or distances, use geographic coordinate systems, but its not great
```
...
#ordering of features is important
#put basemap first and then things you appear on top of that
...
leaflet() %>% 
  addProviderTiles(providers$Esri.WorldShadedRelief) %>%  
                #default is addTiles() which is a basemap
  ...
  addMarkers(data=EPAair_wgs84,popup = ~as.character(`Site.Name`))
    #can tinker around a lot with the types of markers
...
#state is cookie cutter that will cut out any huc that is not in the state boundary)
mapview(huc2_UTM_nc)
```
...
huc0302 <- huc2_utm %>% filter(SUB=='0302')
...
#Read in the demographic data, tabular df only, not spatial
demog_df <- read.csv('./Data/Spatial/NC_Demography.csv') %>% 
  mutate(FIPS = as.factor(FIPS))                                 #Convert the FIPS variable to a factor, in order to retain our leading zeros, because we have to keep the 4 digit code with 0s at the beginning
...
#specify left table (counties_sf_utm) which is x, so join all features that match county features. Then specify the columns between the two datasets that should equal each other.
#make sure to put counties sf first to retain sf feature
#st as sf : tell R that it is a spatial df

#Check the dimensions
dim(counties_sf_utm)
dim(demog_df)
dim(counties_sf_utm_join) #one less than expected because it put GEOID and FIPS into one column
...
triCo_centroids_chull <- triCo_centroids %>% 
  st_union() %>% 
  st_convex_hull()
myMap2 <- myMap + geom_sf(data = triCo_centroids_chull, color = 'red', fill=NA)
myMap2
...
triCo_centroids_voronoi <- triCo_centroids %>% 
  st_union() %>% 
  st_voronoi()
myMap3 <- myMap2 + geom_sf(data = triCo_centroids_voronoi, color = 'purple', fill=NA)
myMap3
...
#User coordinates
#create spatial feature of Grainer Hall
userLat = 36.0045442
userLng = -78.9426381

#Create a simple features point geometry from the point (sfp=spatial feature point)
theSite_sfp <- st_point(c(userLng,userLat))

#Create a simple features column from the point geometry object
#assigning a spatial reference so it knows where it belongs int eh world
...
#intersects is different thann intersection
#what features does our point intersect with
#sparce will return a single column of trues and falses
resultMask <- st_intersects(counties_sf_utm, 
                            theSite_sfc_transformed,
                            sparse = FALSE) #The `sparse` option returns a Boolean mask
#the one true county will contain our point
...
#take coordinate, buffer it with the counties, then intersect that with the counties in the boolean mask
