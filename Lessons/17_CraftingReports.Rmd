...
<These arrows let R Markdown know to not include it in the pdf. These can be secret notes inside R code that won't show up >

- To create a bullet, a dash with a space. You can also use +, *
- Code, output, and text/notes all in one document
- Knit to useful formats
- Share code and output legibly 
- git friendly - version control!
- reproducible
- As a communicaiton tool, what do I want my reader to see, versus what happens behind the scenes. 
- You can focus on output and conclusions, not code. Or vise versa - flexible formating
- Makes it easy for you to format. Simple syntax and autoreferencing.
...
NTLER    |  Chemistry and physical data in lakes
...
getwd()
library(tidyverse) #require will do the same thing, except without any warning or error messages
require(knitr)
library(viridis)
...
Nutrients_processed <- Nutrients %>%
  select(-lakeid, -depth_id, -comments) %>% #for filter, use & and | and for select, use commas
  filter(depth==0)
#View(Nutrients_processed)
```
...
```{r, echo=FALSE}
Nutrients_sum_tn <- Nutrients_processed %>%
  group_by(lakename) %>%
  summarise(mean_tn = mean(tn_ug), 
            min_tn = min(tn_ug),
            max_tn = max(tn_ug),
            sd_tn = sd(tn_ug))
#View(Nutrients_sum_tn)

Nutrients_sum_tp <- Nutrients_processed %>%
  group_by(lakename) %>%
  summarise(mean_tp = mean(tp_ug), 
            min_tp = min(tp_ug),
            max_tp = max(tp_ug),
            sd_tp = sd(tp_ug))
#View(Nutrients_sum_tp)
```
...
```{r, echo=FALSE}
kable(Nutrients_sum_tn, caption = "Total N") 
  
kable(Nutrients_sum_tp, caption = "Total P") 
...
```{r, fig.cap="Total Nitrogen over time", echo=FALSE, warning=FALSE}
total_n <- ggplot(Nutrients_processed, aes(x=sampledate, y=tn_ug)) +
  geom_point(aes(col=lakename)) +
  labs(x= "Date", y= "Total N, ug", color = "Lake Name") +
  scale_color_brewer(palette = "YlGnBu")
total_n
```

```{r, fig.cap="Total Phosphorus over time", echo=FALSE, warning=FALSE}
total_p <- ggplot(Nutrients_processed, aes(x=sampledate, y=tp_ug)) +
  geom_point(aes(col=lakename)) +
  labs(x= "Date", y= "Total P, ug", color = "Lake Name") +
  scale_color_brewer(palette = "YlGnBu")
total_p
```
...
* Table of contents
* Number sections
* Control default size of figures in YAML header
* Citations
