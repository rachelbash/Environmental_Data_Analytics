...
theme_set(mytheme)
#more permanent, good for making lots of plots. Use for remainder of session
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  geom_point() +
  geom_text(x = as.Date("2019-01-01"), y = 45, label = "good", hjust = 1) +
...
print(O3plot7)  
#geom_rect is making a rectangle, given min and max points and the fill color
#geom text adds text based on location (x and y vals), hjust= how close to edge you want, fontface = how bold
#scale_x_date used because our x axis is a date, and tell the starting and ending values, breaks between each date and the label
#within theme, axis.text.x = element_text(angle = 45) 
#brackets give you subscripts
...
NvsP2 <-
  ...
  #xlab("Total Phosphorus, (\U003BCg/L)") +
  #ylab("Total Nitrogen, (\U003BCg/L)") +
  labs(x="Total Phosphorus, (\U003BCg/L)", y = "Total Nitrogen, (\U003BCg/L)", color = "Depth, m", shape = "Lake") +
  scale_shape_manual(values = c(15, 17)) + #specifies shape
  #scale_color_distiller(palette = "Blues", direction = 1) + # use scale_color_brewer for discrete variables
  scale_color_viridis(option = "magma", direction = -1) +
  theme(legend.position = "right", 
        legend.text = element_text(size = 12), legend.title = element_text(size = 12))
print(NvsP2)
#can use paste and expression to add a mu 
...
Nutrientplot <-
  ...
  geom_boxplot() +
  #ylab("Concentration, ug/L") + xlab(NULL)
 labs(x="", y="Concentration (\U003BCg/L)") +
  scale_y_continuous(expand = c(0, 0)) +
  #scale_color_brewer(palette = "YlGnBu") +
scale_color_manual(values = c("#7fcdbb", "#41b6c4", "#1d91c0", "#225ea8", "#0c2c84"), 
                   name = "Nutrient", 
                   breaks = c("nh34", "no23", "po4", "tn_ug", "tp_ug"), 
                   labels = c("Ammonium", "Nitrate", "Phosphorus", "Total N", "Total P")) +
