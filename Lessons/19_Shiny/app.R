...
server <- function(input, output) {
  ...
  output$gdist <- renderPlot({ 
    ...
    p <- ggplot() + scale_x_continuous(limits = c(0, 20))
    p <- p + geom_histogram(aes(x, y = ..density..), bins = bins, colour = 'black', fill = 'white') # add hist if checked
    p <- p + geom_density(aes(x), alpha = 0.2, fill = input$gcol) # add density if checked
    p + theme_minimal()
  })
    # qq plot
