...
class(NTL.phys.data.PeterPaul$sampledate) #reverts back into a factor, loses memory that we changed it to a date. Unfortunate
...
#for numeric characters, blank spaces will autofill with NA, with factors it won't autofill with NA
NTL.nutrient.data.PeterPaul$sampledate <- as.Date(NTL.nutrient.data.PeterPaul$sampledate, format = "%m/%d/%y")

#to only pull up data just from Peter and Paul lakes, and not other lakes, use drop.levels (don't know how to use drop.levels at this point)
...
NTL.phys.data.PeterPaul.skinny <- select(NTL.phys.data.PeterPaul, 
                                         lakename, daynum, year4, sampledate:irradianceDeck) #to remove columns, put a minus sign before the variables
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.skinny, "nutrient", "concentration", tn_ug:po4) #according to set of new columns, put titles of columns into "nutrient" column, and put all the rows of the 5 data into "concentrations"
#gather will take maximum value of concentrations in dataset
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
#remove NAs associated with concentrations
count(NTL.nutrient.data.PeterPaul.gathered, nutrient)
# will fill in missing ones with NAs, unless you remove
...
NTL.nutrient.data.PeterPaul.spread <- spread(NTL.nutrient.data.PeterPaul.gathered, nutrient, concentration) #specify which columns you want to spread by
...
#adds 3 more columns. One for Year, one for month, and one for day. Gets rid of the original date column
...
*x and y being dataset x and dataset y*

* `inner_join`: return rows in x where there are matching values in y, and all columns in x and y (mutating join).
* `semi_join`: return all rows from x where there are matching values in  y, keeping just columns from x (filtering join). #filtering x according to positions that fit in y
...
#lots will be NA because there were a lot of dates that only had phys data and not nutrient data
...
# add a month column to the dataset, add column to very end
...
interval(first(NTL.phys.nutrient.data.PeterPaul$sampledate), last(NTL.phys.nutrient.data.PeterPaul$sampledate)) #more reproducible to do first and last
```
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#grab data frame, THEN, only take depths when it equals 0, THEN separate by lake name, THEN get rid of NAs in ALL temp and tn and tp, THEN give means and stds for each of those measurements. It will give you two means for temps, one for Peter Lake and one for Paul Lake. Gives a new data frame with the new arguments. Helpful for a barchart for example. Could also run a linear regression, t-test, etc
