...
```{r, results = "hide"}
vignette("dplyr")
#shows up in help tab. An R markdown that goes through how to use dplyr
```
...
#take the depth that is equal to 0
# tells R which column to choose. So find depth equals 0 at all columns
...
#with dplyr, R stores this new d.f. with new row numbers
...
#filters out so it only pulls out rows with Paul Lake or Peter Lake
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" 
                                   &  lakename != "Crampton Lake" & lakename != 
                                     "East Long Lake" &
                                     lakename != "Hummingbird Lake" & lakename != 
                                     "Tuesday Lake" &
                                     lakename != "Ward Lake" & lakename != "West 
                                     Long Lake")
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
# %in% means include. Then provide with list of lake. It is inclusive.

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) #between those two dates
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #comma does the same thing
...
NTL.phys.data.19901999Tuesday <- filter(NTL.phys.data, year4 %in% c(1990:1999),
                                        lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> ANSWER: Not easily identifiable. Not efficient to go in and find the rows you wanted
...
head(NTL.phys.data.temp.descending,1)
...
#only gives the variables you want. ONly use commas and colons
```
### Mutate
...
#quicker, can do everything we have done above, but in one argument
```
...
#If false, it won't include the row numbers 1,2,3,...
```
