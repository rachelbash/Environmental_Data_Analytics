...
The coefficients of a hierarchical model are divided into two categories: **fixed effects** and **random effects.** A **fixed effect** is a factor whose levels are experimentally determined or whose interest lies in the effects of each level (e.g., covariates, treatments, interactions). A **random effect** is a factor whose levels are sampled from a larger population, or whose interest lies in the variation among them rather than the specific effect of each level. In choosing whether you are dealing with a fixed or a random effect, consider the following questions: 
  ...
  + Do you have interest in the variance among levels? If so, then random effect
  
  + Do you have interest in generalizing to factor levels that you did not study? IF so, then random effect
...
#this is ANCOVA, fixed effects with interaction is sampledate and lakename
#then specifying random effect
#we're interested in overall trend, but not of interest in understanding change of temp over the weeks
TempTest.auto <- lme(data = PeterPaul.summertemp,
                     temperature_C ~ sampledate * lakename, 
                     random = ~1|Week)
TempTest.auto
#charactarizing fixed effects, AND
#random effect: what is variability among the weeks. Gives us std dev of temp among those weeks. SO 1.5 std dev of temp across those weeks
#14 groups/weeks

ACF(TempTest.auto)
#gives us a bunch of autocorrelation values
#take second value (inner-most group value) to find degree of autocorrelation at first level
```
...
#fixed effects: look familar to previous tests we've ran
#intercept: 20.5 is the temp at Paul lake on the first day of summer
#sampledate: for each day, temp inc by 0.0001. If you multiply 0.001*365*32(total number of years across study) = 1.163. Which is a signif change
...
#come to same conclusion as the model with the random effect. Why do we do one with random effects if there is no difference?

anova(TempTest.mixed, TempTest.fixed)
# The lower the AIC, the better.
# The p-value tells us whether those models have a significantly different fit
#random effects (mixed) accounts for more variability in model structure. Its just a better model. Even though coefficients might be similar, we are accounting for more stuff in the mixed effects number
...
ggplot(PeterPaul.summertemp) +
  ...
  geom_abline(intercept = 20.47, slope = 0.0001, color="red") +
  labs(x = "Date", y = "Temperature (C)") +
  geom_text(x = as.Date("2010-01-01"), y = 15, label = "y = 20.47 + 0.0001x")
