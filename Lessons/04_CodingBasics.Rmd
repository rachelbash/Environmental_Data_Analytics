...
Define what each new piece of syntax does below (i.e., fill in blank comments). Note that the R chunk has been divided into sections (# at beginning of line, ---- at end). Four hyphens after title turn it into a section.

R populates and counts positions of elements vertically automatically. 
C = concatenate 
```{r}
# Vectors ----
...
vector1[3] # pulls out the 3rd item in the first vector(5.3). Matrix subsetting

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #populates matrices vertically first
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #populates matrices by row, horizontally
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #named rows first, then the columns
matrix3["uno",]

matrix1[4, ] #fourth row, all columns
matrix1[ , 3] #third column, all rows
matrix2[c(12, 14)] #calling up data points in position 12 and 14
matrix1[12:14] #calling up data points in position 12 through position 14
...
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) #creating a matrix that has the data in the cells vector, but organize 2x2 with names of rows and columns
matrix4

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]] #for lists, use double square brackets

# Data Frames ----
d <- c(1, 2, 3, 4) # What type of vector?
class(d) #tells you it is numeric
...
dataframe1[1:2] # called up first and second vector in the data frame (columns)
dataframe1[c("ID","Passed")] #called up first and third vectors in dataframe
dataframe1$Color # knows that there are some elements named color. Will pull up those elements with color and give you the list of colors.

```
QUESTION: How do the different types of data appear in the Environment tab?

> ANSWER: They are labeled either data or values. It tells us the name of the element, the type of the element, and it gives you a preview of the data in a list form.
...
> ANSWER: Shows you the various vectors within the dataframe that you can pull up.

QUESTION: What happens when a comment in R is followed by "----"?

> ANSWER: Creates a new section that you can collapse if you want.
...
> ANSWER: Red writing comes up. But its all good
...
#inside parenthesis are recipes
#then in brackets are ingredients
```

A single ingredient recipe: 
```{r}
# Write the recipe, x is incredient
recipe1 <- function(x){
  mix <- x*2
  return(mix)
}

# Bake the recipe, apply to ingredient 5
