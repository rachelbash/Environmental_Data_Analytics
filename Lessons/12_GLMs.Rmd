...
#y var is always on left hand side, response var is always on right hand side
...
#adjusted R squared penalizes you for you dec your degrees of freedom. Goes down the more variables
# you add, the more it will go down
...
> ANSWER: Irradiance can be predicted by depth. For each meter of depth, irradience dec by 96 units (linear regressions; p < 0.0001, df = 15449,  R2 = 0.31). Irradience is predicted at 487 at water surface. Irradience was negatively correlated with depth. You don't need to report an F-statistic

> Add equation of linear regression in figure. There is code to have it appear (look in stack overflow)
...
#not something to put in a final report
```
...
#filter out 0 values because you can't log transform values that are 0
...
#better quantification of that relationship, as shown by inc R squared
...
# SE (standard error-grey bars) can also be removed
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(alpha = 0.5) +
    scale_y_log10() +
    geom_smooth(method = 'lm', se = FALSE, color = "red") +
    labs(x = "Depth (m)", y="Irradience (units)") +
    scale_x_continuous(breaks=c(0,2,4,6,8,10,12)) 
    #geom_text() to add your equation onto the graph. Look up google annotation layer
print(irradiancebydepth2)
#to change transparency of points, alpha = 0.50
#to change the scale of the graph so that it goes up by nicer numbers, scale_x_continuous(breaks=)
...
#significant predictors, but not a good model for predicting total phosphorus
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
#omit NAs so that correlation can be calculated
PeterPaulCorr <- cor(PeterPaulnutrients)
corrplot(PeterPaulCorr, method = "ellipse")
#all have positive correlations among one another, as seen by positive ellipses
...
```{r}
#step-wise addition or subtraction of explanatory variables
...
step(TPAIC)
#starts up at full linear model, then subtracts them one by one
#the lower the better for AIC values
...
#lakename is categorical, depth is continuous

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)
#relationship with depth is different in Peter Lake versus Paul Lake
#for Paul lake there is an inc in Nitrogen as depth
#for peter lake, there is a -48 dec in Nitrogen for each unit of depth
