...
shapiro.test(EPAair$Ozone) #test whether data is close to normal distribution, 
#so if p<0.05, then data is not normally distributed
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram(stat = "count")
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") #specify that alternative hypothesis is less than 50, not just different than 50
O3.onesample
```

What information does the output give us? How might we report this information in a report?

> ANSWER: p-value is less than 0.05, which means we reject the null hypothesis that the mean = 50. , 95% confident that our true mean lies between -Inf and 37. Ozone AQI values in 2017-208 were significantly lower than 50 (one sample t-test; p < 0.001, t = -41.911, df = 1084).
...
var.test(EPAair$Ozone ~ EPAair$Site.Name) #can compare two populations' variances. Give us p-value indicating whether variances are the same. P < 0.05 means that the variances are different from each other

ggplot(EPAair, aes(x = Ozone, color = Site.Name)) +
  geom_freqpoly(stat = "count")

#two ways to run this kind of tes

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Site.Name)
O3.twosample
O3.twosample$p.value #reject null hypothesis that two group means are equal
...
bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name) #test for equal variance for more than two groups of data. # p = 0.08 meaning they are not significantly diff from each other. Equal variance

# Format as a GLM
PM2.5.anova <- lm(EPAair$PM2.5 ~ EPAair$Site.Name)
summary(PM2.5.anova) #only shows us which group is significantly different from the intercept

# Format as an aov
PM2.5.anova2 <- aov(EPAair$PM2.5 ~ EPAair$Site.Name)
summary(PM2.5.anova2) #only tells us whether site is a significant predictor, so must do post-hoc test, but gives us same p-value as lm for overall equation

# Run a post-hoc test for pairwise differences
TukeyHSD(PM2.5.anova2) #tells us the differences between those populations
...
#can see that Bryon and Triple Oak have much closer means (not sig diff from each other)
#can put asterisk over blackstone to show that it is stat sig diff from other two
```
What information does the output give us? How might we report this information in a report?

> ANSWER:(ANOVA; F(2,1898) = 16.16, p < 0.0001)
...
PM2.5.kw #indicates sig diff among values, but doesn't indicate which sites differ
...
summary(TNanova.main) #combo of two of the diff alpha values. Need to add between paul lake + id2 if you want that specific value

TNanova.main2 <- aov(PeterPaul.nutrients$tn_ug ~ PeterPaul.nutrients$lakename + PeterPaul.nutrients$depth_id)
summary(TNanova.main2) #same p-val for depth id. Lake and depth id are sign predictors for actual concentrations of nitrogen levels
...
summary(TNanova.interaction) #there is a sig interaction between lake and nitrogen. Correlation between these two levels. 
...
#3rd level shows all possible combinations
```
