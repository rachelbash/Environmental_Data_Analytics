...
* Manual copy-paste
* API (retrieve data from standard code; data must be in prescribed format) *application programming interface*
* DOM parsing
...
Name #it grabbed what we wanted, but also some things we didn't want
...
# Coerce into a data frame, to ensure consistent lengths across all variables by doing matrix subsetting
riverdata <- data_frame(Name = Name[6:188], #take out the parts of it that aren't in the table
                        Length.km = Length.km[2:184], 
                        DrainageArea.km2 = DrainageArea.km2[2:184], 
                        Discharge.m3s = Discharge.m3s[2:184], 
                        Outflow = Outflow)

# Remove unnecessary text from within cells, because this is a messy dataset
riverdata$Name <- str_replace(riverdata$Name, "\\[.*\\]", "") #remove anything inside square brackets including the square brackets. \\ says spaces or anything
...
#get rid of commas using substitute command. Replace comma with nothing!
#this also put NAs in all blank spaces in the table
